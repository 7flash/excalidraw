...
| `skipSnapshotUpdate` | `boolean` | Implies whether the `store` should skip update of it's elements snapshot, which is necessary for correct diff calculation. Relevant only when `elements` are passed in, and gets overriden when `commitToStore` is set to `true`. Defaults to `false`. |
