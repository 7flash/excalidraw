...
| `commitToStore` | `boolean` | Implies if the `store` should record the update. Recorded changes are emmitted and listened to by other components, such as `History` for undo / redo purposes. Defaults to `false`. |
| `isRemoteUpdate` | `boolean` | Implies whether the update is from remote client. Defaults to `false`. |
