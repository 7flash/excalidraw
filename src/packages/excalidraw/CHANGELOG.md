...
- Rare updates of `elements` performed through [`updateScene`](https://github.com/excalidraw/excalidraw/blob/master/src/components/App.tsx#L282) without `commitToStore` set to `true` require a new parameter `skipSnapshotUpdate` to be set to `true`, if the given update should be locally undo-able with the next user action. In most cases such a parameter shouldn't be needed, i.e. during multiplayer collab updates, which shouldn't alter the local history.
